...
function Table({ title, label, rows }: {
  ...
  return (
    <div className="rounded-xl border bg-white shadow-sm overflow-hidden" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
      <div className="px-4 py-3 font-bold" style={{ color: 'var(--text)' }}>{t(title)}</div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left border-y" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>
              ...
              <th className="px-2 py-2 text-right">{t('Objectif')}</th>
              <th className="px-2 py-2 text-right">{t('Réalisé')}</th>
              <th className="px-2 py-2 text-right">{t('R/O')}</th>
              <th className="px-4 py-2 text-right">{t('Reste')}</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-6 text-center" style={{ color: 'var(--text-muted)' }}>{t('Aucun objectif saisi — renseignez-les dans Paramétrage')}</td></tr>
            ) : rows.map((r) => (
...
export function DashboardObjectifs() {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-base font-black" style={{ color: 'var(--text)' }}>{t('Suivi des objectifs de recrutement — ce mois')}</h3>
